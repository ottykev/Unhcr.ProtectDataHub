...
namespace Unhcr.ProtectDataHub.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("ProtectDataHub") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "ProtectDataHub", DisplayName = "ProtectDataHub API", Resources = { "ProtectDataHub" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["ProtectDataHub_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["ProtectDataHub_Web:RootUrl"]!.EnsureEndsWith('/');

            /* ProtectDataHub_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["ProtectDataHub_Web:ClientSecret"] ?? "1q2w3e*",
